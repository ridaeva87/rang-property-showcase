...
## 26. Актуализация после Этапа 3

Для Этапа 3 проанализированы все четыре вкладки предоставленного файла `Характеристики помещений.xlsx`. Полное соответствие полей и список неоднозначностей находятся в `EXCEL_PROPERTY_MAPPING.md`. Массовый импорт не выполнялся.

Модель `Property` расширена полями полезной площади, независимых финансовых показателей, высоты, отопления, электроснабжения, 220/380 В, мощности и её увеличения, санузла, ворот, кондиционирования, материала, транспортного доступа, нескольких фотографий и видео. Пустые значения остаются `undefined` и не выводятся.

Объектовые данные отделены от помещения: `Property.objectId` ссылается на `PropertyObject`. Адрес, описание территории и правила парковки не дублируются по карточкам. Для `АК 153А` закреплён подтверждённый адрес «Аделя Кутуя, 153А» и согласованный текст парковки; у других объектов он не отображается.

Из Excel точечно применена только однозначно сопоставленная строка `А1 Склад №8`, 133 м². Ставка хранится как `rentPricePerSqm = 1260`, коммунальные расходы — отдельно. Полная месячная стоимость не вычисляется. Неясные единицы электрической мощности явно помечены как требующие уточнения.

Подробная карточка использует существующий маршрут `/properties/$slug`, единый `PROPERTIES` и систему избранного Этапа 2. Характеристики сгруппированы; пустые группы отсутствуют. `PropertyGallery` поддерживает основное фото и переключение дополнительных; при отсутствии подтверждённых фото показывает нейтральное состояние. `Property.video` поддерживает direct file и embed, но плеер не выводится без реального URL.

Backend, БД, отправка CTA, загрузка медиа, массовый Excel-импорт и административное редактирование по-прежнему отсутствуют.
