...
## 27. Актуализация после Этапа 4

Этап 4 продолжает существующую статическую frontend-архитектуру. Backend, база данных, авторизация, система заявок, Object Storage и внешние интеграции не добавлялись. Production target остаётся Nitro `node-server` на VPS Timeweb Cloud.

- Добавлены индексируемые разделы `/rent`, `/sale`, `/services`, `/objects`, `/objects/$slug` и `/about`. Главная страница и навигация ведут в эти разделы без дублирования существующих секций.
- Аренда использует существующий каталог и его query-параметр `type`. Условия, сроки и расходы не фиксируются как подтверждённые, если их нет в данных конкретного помещения.
- В `Property` добавлено независимое поле `offerType: "rent" | "sale"`; арендный `status` больше не является обязательным для будущих предложений продажи. Цена и статус продажи хранятся отдельно от арендной ставки и статусов. Реальные объекты продажи не предоставлены, поэтому `/sale` показывает честное пустое состояние.
- `PropertyObject` хранит `id`, `slug`, адрес, описание, координаты, фотографии, особенности территории, парковку и режим доступа. Помещения ссылаются на объект через `objectId`; их данные внутри объекта не копируются.
- Текст парковки для Аделя Кутуя, 153А хранится один раз на уровне объекта и доступен его странице и связанным карточкам помещений.
- `ADDITIONAL_SERVICES` задаёт единую расширяемую структуру согласованных категорий. Цены, описания и условия отсутствуют до получения подтверждённых данных. CTA не отправляют и не сохраняют заявки.
- Карта реализована как честное подготовленное состояние со списком объектов и переходами. Координаты, API-ключ и картографический сервис отсутствуют, поэтому фиктивная интерактивная карта не создавалась.
