...
if (!path || !process.env.DATABASE_URL)
  throw new Error("manifest path and DATABASE_URL are required");
...
try {
  ...
  for (const item of items) {
    ...
    await db.query(
      `insert into ${table} (${target},media_id,sort_order) values ($1,$2,$3) on conflict (${target},media_id) do update set sort_order=excluded.sort_order`,
      [item.targetId, item.id, item.sortOrder],
    );
  }
  await db.query("commit");
  console.log(
    JSON.stringify({
      mediaAssets: items.length,
      premiseLinks: items.filter((x) => x.link === "premise").length,
      objectLinks: items.filter((x) => x.link === "object").length,
    }),
  );
} catch (error) {
