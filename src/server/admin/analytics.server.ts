...
export async function statisticsAdmin(input:{period?:string|undefined;from?:string|undefined;to?:string|undefined}){
  ...
  return{period:{from:from.toISOString().slice(0,10),to:to.toISOString().slice(0,10),label},stats:{premises:Number(counts.premises),published:Number(counts.published),tenants:Number(tenantCount.value),requests:Number(requestCount.value),views:eventTotals["premise_view"]||0,favorites:eventTotals["favorite_add"]||0,cta:eventTotals["premise_cta"]||0,leads:eventTotals["premise_lead"]||0,portalLogins:eventTotals["portal_login"]||0},requestByCategory,requestByObject,requestByPremise,requestDynamics,requestSources,popular,interests,serviceUsage,serviceCategories,adStats};
}
