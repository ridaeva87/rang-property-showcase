...
export const adLeads = pgTable(
  "ad_leads",
  {
    ...
    cta: text("cta").default("contact_form").notNull(),
    inquiryType: text("inquiry_type").default("internal_form").notNull(),
    status: text("status").default("new").notNull(),
    assigneeEmployeeId: text("assignee_employee_id").references(() => employees.id, { onDelete: "set null" }),
    ...timestamps,
  },
...
export const meters = pgTable(
  "meters",
  {
    ...
    name: text("name"),
    isActive: boolean("is_active").default(true).notNull(),
    isMonotonic: boolean("is_monotonic").default(true).notNull(),
...
export const expenses = pgTable(
  "expenses",
  {
    ...
    currency: text("currency").default("RUB").notNull(),
    note: text("note"),
    tenantUserId: text("tenant_user_id").references(() => users.id, { onDelete: "restrict" }),
    premiseId: text("premise_id").references(() => premises.id, { onDelete: "restrict" }),
    consumption: decimal("consumption", { precision: 16, scale: 4 }),
    ...timestamps,
  },
